...
globalThis.fetch = async url => {
  ...
  if (value.includes(`/tokens/${mint}/pools`)) return response({
    data:[{ type:'pool',id:`solana_${poolAddress}`,attributes:{ address:poolAddress,base_token_price_usd:'2',quote_token_price_usd:'1',reserve_in_usd:'1000000',volume_usd:{h24:'500000'} },relationships:{ base_token:{data:{id:`solana_${mint}`,type:'token'}},quote_token:{data:{id:`solana_${quoteMint}`,type:'token'}},dex:{data:{id:'edge-dex',type:'dex'}} } }],
    included:[{ type:'token',id:`solana_${mint}`,attributes:{address:mint,name:'Edge Token',symbol:'EDGE'} }]
  });
