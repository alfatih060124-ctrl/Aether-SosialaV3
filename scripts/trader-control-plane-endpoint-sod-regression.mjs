...
const sharedAdmin = await invoke({
  method: 'POST', parts: ['api','admin','traders','trader-1','evidence'], token: env.ADMIN_API_TOKEN, actor: 'admin.operator',
  body: { source_type: 'MANUAL', source_reference: 'ref-1', observed_at: '2026-09-05T00:00:00Z' },
});
assert.equal(sharedAdmin.handled, true);
assert.equal(sharedAdmin.response.status, 401, 'shared admin token must not authorize sensitive mutation');
assert.equal(audit.length, 0);

const evidence = await invoke({
  method: 'POST', parts: ['api','admin','traders','trader-1','evidence'], token: env.TRADER_EVIDENCE_RECORDER_API_TOKEN, actor: 'evidence.operator',
  body: { source_type: 'MANUAL', source_reference: 'ref-1', observed_at: '2026-09-05T00:00:00Z' },
});
assert.equal(evidence.response.status, 201);
assert.equal(evidence.response.payload.live_execution_authorized, false);
assert.equal(audit[0].actor, 'evidence.operator');
assert.equal(audit[0].payload.actor_role, 'TRADER_EVIDENCE_RECORDER');
assert.equal(audit[0].payload.execution_dispatched, false);

const actorReuseVerify = await invoke({
  method: 'PATCH', parts: ['api','admin','traders','trader-1','verification'], token: env.TRADER_VERIFIER_API_TOKEN, actor: 'evidence.operator',
  body: { decision: 'VERIFY', evidence_id: 'evidence-1' },
});
assert.equal(actorReuseVerify.response.status, 403);
assert.equal(verified, false);

const verification = await invoke({
  method: 'PATCH', parts: ['api','admin','traders','trader-1','verification'], token: env.TRADER_VERIFIER_API_TOKEN, actor: 'verification.operator',
  body: { decision: 'VERIFY', evidence_id: 'evidence-1' },
});
assert.equal(verification.response.status, 200);
assert.equal(verification.response.payload.publication_authorized, false);
assert.equal(audit[1].actor, 'verification.operator');
assert.equal(audit[1].payload.actor_role, 'TRADER_VERIFIER');
assert.equal(audit[1].payload.evidence_id, 'evidence-1');
assert.equal(audit[1].payload.verified, true);

const actorReusePublish = await invoke({
  method: 'PATCH', parts: ['api','admin','traders','trader-1','publication'], token: env.TRADER_PUBLISHER_API_TOKEN, actor: 'verification.operator',
  body: { published: true },
});
assert.equal(actorReusePublish.response.status, 403);

const publication = await invoke({
  method: 'PATCH', parts: ['api','admin','traders','trader-1','publication'], token: env.TRADER_PUBLISHER_API_TOKEN, actor: 'publication.operator',
  body: { published: true },
});
assert.equal(publication.response.status, 200);
assert.equal(publication.response.payload.trader.published, true);
assert.equal(publication.response.payload.live_execution_authorized, false);
assert.equal(audit[2].actor, 'publication.operator');
