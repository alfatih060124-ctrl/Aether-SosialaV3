...
const tokenMint = 'TOKEN_MINT_TEST';
const quoteMint = 'USDC_MINT_TEST';
...
const poolQuote = dex => async request => ({
  ...
  quote_source: dex === 'orca' ? 'ORCA_WHIRLPOOLS_ONCHAIN_RPC_TEST' : 'RAYDIUM_ONCHAIN_RPC_CPMM_TEST',
  quote_verified: true,
  costs_verified: true,
    ...
    async json() {
      ...
      return {
        ...
        data: {
          data: [{
            id: 'raydium-pool',
            programId: raydiumCpmmProgram,
            mintA: { address: tokenMint },
            mintB: { address: quoteMint },
            type: 'CPMM',
            tvl: 1_000_000
          }]
        }
...
const runtime = createOrcaRaydiumShadowRuntime({
...
assert.deepEqual(new Set(scan.opportunities.map(item => item.direction)), new Set(['ORCA_TO_RAYDIUM', 'RAYDIUM_TO_ORCA']));
