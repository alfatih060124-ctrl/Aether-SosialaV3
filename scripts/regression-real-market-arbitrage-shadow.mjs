...
const route = (overrides = {}) => ({
  pool_address: 'pool-a', dex_id: 'dex-a', price_usd: 1,
...
const risk = {
  ...
  token_age_hours: 240, route_count: 6, source_count: 5,
...
const opportunity = {
  ...
  market_source: 'REAL_MARKET_TEST', network_fee_usdc: 0.01,
  buy_route: route(),
  sell_route: route({ pool_address: 'pool-b', dex_id: 'dex-b', price_usd: 1.01 })
};
...
assert.equal(sim.buy_route.pool_address, 'pool-a');
assert.equal(sim.sell_route.pool_address, 'pool-b');
...
const below = evaluateRealMarketArbitrageShadow({ opportunity: { ...opportunity, sell_route: route({ pool_address: 'pool-b', dex_id: 'dex-b', price_usd: 1.001 }) }, notional_usdc: 1000, risk_evidence: risk, now });
