...
assert.equal(evidence.collection_status,'PENDING_DATA');assert.equal(evidence.account_exists,true);assert.equal(evidence.account_owner,OWNER);assert.equal(evidence.account_executable,false);assert.equal(evidence.account_space,0);assert.equal(evidence.source_reference,null);
...
assert.equal(missing.account_exists,false);assert.equal(missing.account_owner,null);assert.equal(missing.account_executable,null);assert.equal(missing.account_space,null);assert.equal(verifySolanaWalletAccountIdentitySnapshot(missing),true);
...
for(const mutate of [e=>{e.verified=true;},e=>{e.published=true;},e=>{e.metrics_available=true;},e=>{e.trades_count=1;},e=>{e.source_reference='solana_rpc:synthetic@1';},e=>{e.account_owner=WALLET;},e=>{e.provenance.context_slot=999;},e=>{e.provenance.account_space=7;},e=>{e.provenance.data_slice.length=1;},e=>{e.provenance.source_reference_policy='TRANSACTION';}]){const t=clone(evidence);mutate(t);assert.equal(verifySolanaWalletAccountIdentitySnapshot(t),false);}
