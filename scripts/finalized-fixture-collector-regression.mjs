...
function transaction({ programId = PROGRAM_ID, slot = 123456, blockTime = 1788257000, metaErr = null } = {}) {
  return {
    slot,
    blockTime,
    meta: {
...
function rpcMock({ status = 'finalized', statusErr = null, tx = transaction() } = {}) {
...
assert.equal(captured.fixture_class, 'VERIFIED_ONCHAIN');
assert.equal(captured.capture_kind, 'SOLANA_FINALIZED_TRANSACTION');
assert.equal(captured.review_state, 'RAW_CAPTURE');
