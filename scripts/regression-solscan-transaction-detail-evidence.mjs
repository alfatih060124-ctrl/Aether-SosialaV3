...
// Match Solscan's native shape: no echoed tx_hash; integer balance values may be JSON numbers.
const detail={block_id:100,block_time:1767225601,fee:5000,status:1,compute_units_consumed:12345,priority_fee:1000,signer:[W],programs_involved:[P],sol_bal_change:[{address:W,pre_balance:1000000000,post_balance:999995000,change_amount:-5000},{address:O,pre_balance:1,post_balance:2,change_amount:1}],token_bal_change:[{address:TA,token_address:M,change_type:'inc',change_amount:10,decimals:6,pre_balance:100,post_balance:110,owner:W,pre_owner:W,post_owner:W}]};
...
assert.equal(e.schema,'aether.solscan.transaction_detail_evidence.v2');
assert.equal(e.collection_status,'PENDING_DATA');assert.equal(e.metrics_available,false);assert.equal(e.verified,false);assert.equal(e.published,false);assert.equal(e.live_execution_authorized,false);assert.equal(e.reconciliation_required,true);assert.equal(e.trades_count,null);assert.equal(e.total_return_bps,null);assert.equal(e.win_rate_bps,null);assert.equal(e.drawdown_bps,null);assert.equal(e.reputation_score,null);assert.equal(e.calculation_hash,null);assert.equal(e.source_reference,`solscan:transaction:${SIG}@100`);assert.equal(e.provenance.source_reference_policy,'PROVIDER_VALIDATED_SIGNATURE_SLOT');assert.equal(e.row.signature,SIG);assert.equal(e.row.sol_balance_changes.length,1);assert.equal(e.row.token_balance_changes.length,1);assert.equal(e.row.sol_balance_changes[0].pre_balance,'1000000000');assert.equal(e.row.sol_balance_changes[0].change_amount,'-5000');assert.equal(e.row.token_balance_changes[0].pre_balance,'100');assert.equal(e.row.token_balance_changes[0].change_amount,'10');assert.equal(verifySolscanTransactionDetailEvidence(e),true);
const missing=await collect(null);assert.equal(missing.source_reference,null);assert.equal(missing.row.found,false);assert.equal(missing.calculation_hash,null);assert.equal(verifySolscanTransactionDetailEvidence(missing),true);
for(const [field,value] of [['verified',true],['published',true],['live_execution_authorized',true],['trades_count',1],['total_return_bps',100],['win_rate_bps',5000],['drawdown_bps',100],['reputation_score',99],['calculation_hash','0'.repeat(64)]]){const x=clone(e);x[field]=value;assert.equal(verifySolscanTransactionDetailEvidence(x),false,field);}
...
{const x=clone(e);x.provenance.source_reference_policy='CALLER_SUPPLIED';x.source_hash='0'.repeat(64);assert.equal(verifySolscanTransactionDetailEvidence(x),false);}
...
// Provider echo is optional. If an adapter supplies one, it must still match the requested signature.
await assert.rejects(()=>collect({...detail,tx_hash:SIG2}),/transaction_signature_mismatch/);
const echoed=await collect({...detail,tx_hash:SIG});assert.equal(echoed.row.signature,SIG);assert.equal(verifySolscanTransactionDetailEvidence(echoed),true);
...
await assert.rejects(()=>collect({...detail,sol_bal_change:[{address:W,pre_balance:Number.MAX_SAFE_INTEGER+1,post_balance:90,change_amount:-9}]}),/invalid_sol_pre_balance/);
await assert.rejects(()=>collect({...detail,token_bal_change:[{...detail.token_bal_change[0],pre_balance:Number.MAX_SAFE_INTEGER+1}]}),/invalid_token_pre_balance/);
...
console.log(JSON.stringify({ok:true,fixture_policy:'SYNTHETIC_TEST_ONLY',schema:e.schema,tests:34,collection_status:e.collection_status,metrics_available:e.metrics_available,verified:e.verified,published:e.published,calculation_hash:e.calculation_hash}));
