...
for(const required of [
  'Automatic Solana + Reconciled Performance',
...
assert(/evidence\/reconcile'\s*,\s*\{method:'POST',body:'\{\}'\}/.test(html),'performance builder must not accept client-supplied metrics');
...
assert(!/buildReconciledPerformanceEvidence[\s\S]{0,1000}setPublication\(/.test(html),'performance build must not publish');
