...
import { corroborateAutomaticEvidenceV4, verifyAutomaticEvidenceCorroborationV4 } from '../services/api/src/automatic-evidence-corroboration-v4.mjs';
...
async function rejects(code, options = {}) {
  ...
  assert.throws(
    () => corroborateAutomaticEvidenceV4({ rpcEvidence: rpc, solscanEvidence: solscan, observedAt }),
...
const receipt = corroborateAutomaticEvidenceV4({ rpcEvidence: rpc, solscanEvidence: solscan, observedAt: CORROBORATED_AT });
assert.equal(receipt.schema, 'aether.automatic_evidence.cross_source_corroboration.v4');
...
assert.equal(verifyAutomaticEvidenceCorroborationV4(receipt), true);
...
for (const mutate of [
  ...
  mutate(tampered);
  assert.equal(verifyAutomaticEvidenceCorroborationV4(tampered), false);
}

console.log('automatic evidence cross-source corroboration v4 regression: ok');
