...
const wrongBindingRepo = createAutoTradeRuntimeRiskRepository({ async query() { return { rows: [{
  ...await db.query('', [policy, follower]).then((r) => r.rows[0]),
  follower_user_id: '33333333-3333-4333-8333-333333333333'
}] }; } });
const wrong = await wrongBindingRepo.getRuntimeRiskSnapshot({ authenticated_follower_user_id: follower, policy_id: policy });
assert.notEqual(wrong.authenticated_follower_user_id, follower);
