...
const base = { initial_balance_usdc: 100, cash_balance_usdc: 100, open_position: {} };
const profitable = settleDemoArbitrage({ account: base, notionalUsdc: 100, finalUsdc: 101, performanceFeeBps: 1000 });
...
const loss = settleDemoArbitrage({ account: base, notionalUsdc: 50, finalUsdc: 49.5, performanceFeeBps: 1000 });
...
await assert.rejects(async () => settleDemoArbitrage({ account: { ...base, cash_balance_usdc: 90, open_position: { notional_usdc: 10 } }, notionalUsdc: 10, finalUsdc: 10.1 }), /arbitrage_open_position_not_allowed/);
console.log(JSON.stringify({ ok: true, schema: 'aether.member_autotrade_demo.regression.v2', mode: 'SHADOW', strategy: 'TWO_LEG_ARBITRAGE', live_execution_authorized: false }));
