...
assert.doesNotMatch(
  source,
  /Math\.max\(0,\s*Number\(process\.env\.SIGNAL_MAX_TOP10_HOLDER_PCT\s*\|\|/,
  'SIGNAL_MAX_TOP10_HOLDER_PCT must not use fail-open NaN parsing'
);

assert.match(
  source,
  /Number\.isFinite\(/,
  'shadow probe holder threshold config must explicitly reject or safely fallback non-finite values'
);
assert.match(source, /holder_gate_passed:/, 'holder gate must remain explicit');
...
console.log('shadow probe holder threshold config regression: PASS');
