...
BEGIN
  ...
  VALUES(kind, aether_audit_actor(), 'TRADER', NEW.trader_id::text,
    jsonb_build_object('before', jsonb_build_object('onboarding_status', OLD.onboarding_status,'verification_status', OLD.verification_status,'verified', OLD.verified,'published', OLD.published,'status', OLD.status,'mode', OLD.mode),
                       'after', jsonb_build_object('onboarding_status', NEW.onboarding_status,'verification_status', NEW.verification_status,'verified', NEW.verified,'published', NEW.published,'status', NEW.status,'mode', NEW.mode),
...
CREATE TRIGGER trg_aether_audit_trader_control_plane AFTER UPDATE ON traders FOR EACH ROW EXECUTE FUNCTION aether_audit_trader_control_plane();

CREATE OR REPLACE FUNCTION aether_guard_copy_mandate()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NEW.mode IS DISTINCT FROM 'SHADOW' THEN RAISE EXCEPTION 'copy_mandate_mode_must_remain_shadow'; END IF;
  IF NEW.live_execution_authorized IS DISTINCT FROM false THEN RAISE EXCEPTION 'copy_mandate_live_execution_forbidden'; END IF;
  IF NEW.allocation_bps < 0 OR NEW.allocation_bps > 10000 THEN RAISE EXCEPTION 'invalid_allocation_bps'; END IF;
  IF NEW.max_slippage_bps < 0 OR NEW.max_slippage_bps > 10000 THEN RAISE EXCEPTION 'invalid_max_slippage_bps'; END IF;
  IF NEW.max_daily_loss_bps < 0 OR NEW.max_daily_loss_bps > 10000 THEN RAISE EXCEPTION 'invalid_max_daily_loss_bps'; END IF;
  IF NEW.stop_drawdown_bps < 0 OR NEW.stop_drawdown_bps > 10000 THEN RAISE EXCEPTION 'invalid_stop_drawdown_bps'; END IF;
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_aether_guard_copy_mandate ON copy_policies;
CREATE TRIGGER trg_aether_guard_copy_mandate BEFORE INSERT OR UPDATE ON copy_policies FOR EACH ROW EXECUTE FUNCTION aether_guard_copy_mandate();

CREATE OR REPLACE FUNCTION aether_audit_copy_mandate()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO audit_events(event_type, actor, entity_type, entity_id, payload)
  VALUES(CASE WHEN TG_OP = 'INSERT' THEN 'COPY_MANDATE_CREATED' ELSE 'COPY_MANDATE_CHANGED' END, aether_audit_actor(), 'COPY_MANDATE', NEW.policy_id::text,
    jsonb_build_object('follower_user_id', NEW.follower_user_id,'trader_id', NEW.trader_id,'enabled', NEW.enabled,'status', NEW.status,'mode', NEW.mode,'allocation_bps', NEW.allocation_bps,'max_slippage_bps', NEW.max_slippage_bps,'max_daily_loss_bps', NEW.max_daily_loss_bps,'stop_drawdown_bps', NEW.stop_drawdown_bps,'live_execution_authorized', NEW.live_execution_authorized));
  RETURN NEW;
END;
$$;
DROP TRIGGER IF EXISTS trg_aether_audit_copy_mandate ON copy_policies;
CREATE TRIGGER trg_aether_audit_copy_mandate AFTER INSERT OR UPDATE ON copy_policies FOR EACH ROW EXECUTE FUNCTION aether_audit_copy_mandate();
