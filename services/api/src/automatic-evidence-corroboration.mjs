...
function envelope(provenance) {
  return {
    schema: 'aether.automatic_evidence.cross_source_corroboration.v1',
...
export function corroborateAutomaticEvidence({ rpcEvidence, solscanEvidence, observedAt }) {
  ...
  return envelope({
    schema: 'aether.automatic_evidence.cross_source_corroboration.provenance.v1',
    algorithm: 'EXACT_SIGNATURE_SLOT_MATCH_V1',
...
export function verifyAutomaticEvidenceCorroboration(evidence) {
  if (!plain(evidence) || evidence.schema !== 'aether.automatic_evidence.cross_source_corroboration.v1') return false;
  try {
    ...
    const p = evidence.provenance;
    if (!plain(p) || p.schema !== 'aether.automatic_evidence.cross_source_corroboration.provenance.v1' || p.algorithm !== 'EXACT_SIGNATURE_SLOT_MATCH_V1') return false;
    ...
    if (p.rpc_upstream_source_hash !== null && (typeof p.rpc_upstream_source_hash !== 'string' || !/^[0-9a-f]{64}$/.test(p.rpc_upstream_source_hash))) return false;
