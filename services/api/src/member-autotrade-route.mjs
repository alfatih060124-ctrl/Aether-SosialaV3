  ...
  try {
    await repos.auditEvents.append({
      ...
      payload: {
        ...
        min_expected_net_edge_bps: Number.isFinite(Number(scan.min_expected_net_edge_bps)) ? Number(scan.min_expected_net_edge_bps) : 0.5,
        mode: 'SHADOW',
  ...
  if (route === PAPER_PERFORMANCE_ROUTE) {
    ...
    try {
      ...
      send(res, statusFor(error), {
        ...
        mode: 'SHADOW', funds_moved: false, live_execution_authorized: false
      });
      return true;
    }
  }
  ...
  if (route === MARKET_SHADOW_ROUTE) {
    ...
    try {
      ...
      send(res, req.method === 'POST' ? 202 : 200, {
        ...
        min_expected_net_edge_bps: Number.isFinite(Number(scan.min_expected_net_edge_bps)) ? Number(scan.min_expected_net_edge_bps) : 0.5,
        paper_position_size_pct: Math.max(1, Math.min(100, Number(process.env.AUTOTRADE_PAPER_POSITION_SIZE_PCT || 25))),
        scanner_sla_ms: 300,
        paper_execution_sla_ms: 3000,
        execution_dispatched: false,
        funds_moved: false,
        transaction_signed: false,
        network_submission_authorized: false,
