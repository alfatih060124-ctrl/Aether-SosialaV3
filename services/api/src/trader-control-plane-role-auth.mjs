...
export const TRADER_CONTROL_PLANE_ROLE_AUTH_CONTRACT = Object.freeze({
  ...
  roles: Object.freeze({ EVIDENCE: 'TRADER_EVIDENCE_RECORDER', VERIFICATION: 'TRADER_VERIFIER', PUBLICATION: 'TRADER_PUBLISHER' }),
});
...
function actorHeader(req) { return String(req?.headers?.['x-aether-actor'] || '').trim(); }
function sameSecret(left, right) { if (!left || !right) return false; const a = Buffer.from(String(left)); const b = Buffer.from(String(right)); return a.length === b.length && timingSafeEqual(a, b); }
function configuredSecrets(env) { return Object.fromEntries(Object.entries(ROLE_CONFIG).map(([key, config]) => [key, String(env?.[config.env] || '').trim()])); }

export function validateTraderControlPlaneRoleConfig(env = process.env) {
  const secrets = configuredSecrets(env);
  for (const [key, config] of Object.entries(ROLE_CONFIG)) if (!secrets[key]) throw new Error(`trader_control_plane_${config.role.toLowerCase()}_credential_required`);
...
export function createTraderControlPlaneRoleAuth({ env = process.env } = {}) {
  ...
  function authorize(req, capability, history = {}) {
    ...
    if (!sameSecret(bearerToken(req), secrets[capability])) throw new Error('trader_control_plane_role_unauthorized');
    ...
    return Object.freeze({ contract: TRADER_CONTROL_PLANE_ROLE_AUTH_CONTRACT.contract, actor, role: config.role, capability, credential_source: config.env, caller_authority: false, shared_admin_token_authority: false, execution_dispatched: false, network_submission_authorized: false, live_execution_authorized: false, signer_required: false, mode: 'SHADOW' });
  }
