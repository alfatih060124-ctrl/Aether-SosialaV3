...
export function createMemberAutoTradeRealMarketRuntime({ discoveryService, qualificationRuntime, quoteMint, discoveryView = 'trending', maxCandidates = 20 } = {}) {
  ...
  return Object.freeze({
    async runNextOpportunity({ demo_account } = {}) {
      ...
      for (const item of discovery.items.slice(0, limit)) {
        ...
        if (!seen.has(key)) { seen.add(key); candidates.push(pair); }
      }
      if (!candidates.length) {
        return Object.freeze({ selected: null, qualified_count: 0, candidate_count: 0, market_source: 'ORCA_RAYDIUM_VERIFIED_FROM_READONLY_DISCOVERY', discovery_source: discovery.source || 'GECKOTERMINAL_PUBLIC', discovery_execution_ready: false, mode: 'SHADOW', strategy: 'TWO_LEG_ARBITRAGE', execution_dispatched: false, funds_moved: false, network_submission_authorized: false, live_execution_authorized: false });
      }
      const scans = [];
      for (const pair of candidates) {
        try {
          const result = await qualificationRuntime.scanAndQualifyPair({ ...pair, demo_account });
          if (result?.mode !== 'SHADOW' || result?.strategy !== 'TWO_LEG_ARBITRAGE' || result?.live_execution_authorized !== false) throw new Error('member_real_market_shadow_invariant_failed');
          scans.push(result);
        } catch (error) {
          const code = String(error?.message || '');
          if (/no_exact_pair|both_dexes_required|no_verified_quotes|pair_mismatch/.test(code)) continue;
          throw error;
        }
      }
      const qualified = scans.flatMap(scan => (scan.results || []).filter(item => item?.qualified === true && item?.assessment?.decision?.action === 'ARBITRAGE_SETTLE'));
      qualified.sort((a,b) => expectedNetEdge(b) - expectedNetEdge(a));
      return Object.freeze({ selected: qualified[0] || null, qualified_count: qualified.length, candidate_count: candidates.length,
        market_source: 'ORCA_RAYDIUM_VERIFIED_FROM_READONLY_DISCOVERY', discovery_source: discovery.source || 'GECKOTERMINAL_PUBLIC', discovery_execution_ready: false,
        mode: 'SHADOW', strategy: 'TWO_LEG_ARBITRAGE', execution_dispatched: false, funds_moved: false, network_submission_authorized: false, live_execution_authorized: false });
    }
  });
}

export const MEMBER_AUTOTRADE_REAL_MARKET_RUNTIME = Object.freeze({
  mode: 'SHADOW', strategy: 'TWO_LEG_ARBITRAGE', dex_scope: Object.freeze(['ORCA','RAYDIUM']),
  candidate_discovery_is_execution_evidence: false, min_expected_net_edge_bps: 20, transaction_signing_authorized: false,
  private_key_allowed: false, network_submission_authorized: false, live_execution_authorized: false
});
