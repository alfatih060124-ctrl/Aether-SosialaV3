...
const pathParts = (url) => new URL(url,'http://localhost').pathname.split('/').filter(Boolean);
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && parts[1] === 'trades') return send(res, 200, {items: await repos.tradeEvents.recent(new URL(req.url,'http://localhost').searchParams.get('limit'))});
    if (req.method === 'GET' && parts[1] === 'traders') {
      if (parts[2]) { const trader = await repos.marketplace.getTrader(parts[2]); return trader ? send(res,200,trader) : send(res,404,{error:'trader_not_found'}); }
      return send(res,200,{items:await repos.marketplace.listTraders()});
    }
    ...
    if (req.method === 'PATCH' && parts[1] === 'admin' && parts[2] === 'copy-policies' && parts[3]) {
      const updated = await repos.admin.updateCopyPolicy(parts[3], await jsonBody(req));
