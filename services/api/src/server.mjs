...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(req.method==='PATCH'&&req.url==='/api/admin/fees'){
   ...
   await repos.auditEvents.append({event_type:'PLATFORM_FEE_CONFIG_UPDATED',actor:'admin',entity_type:'platform_fee_config',entity_id:String(config.config_id),payload:{performance_fee_bps:config.performance_fee_bps,execution_fee_bps:config.execution_fee_bps,enabled:config.enabled}});
