export async function getMemberEngineRentalState(pool, userId, now = new Date()) {
  ...
  const plans = await pool.query(`
    SELECT plan_id,code,name,billing_period_days,price_usdc,base_price_usdc,discount_bps,currency,payment_network,active
    FROM member_engine_plans
    WHERE active=true
    ORDER BY billing_period_days ASC, created_at ASC
  `);
  ...
  const q = await pool.query(`
    SELECT s.subscription_id,s.user_id,s.plan_id,s.status,s.payment_status,
           ...
           p.base_price_usdc,p.discount_bps,p.currency,p.payment_network
  ...
  if (!subscription) return { allowed:false, reason:'engine_subscription_required', subscription:null, available_plans, payment_asset:'USDC', payment_network:'SOLANA' };
  if (subscription.status !== 'ACTIVE') return { allowed:false, reason:`engine_subscription_${String(subscription.status).toLowerCase()}`, subscription, available_plans, payment_asset:'USDC', payment_network:'SOLANA' };
  if (subscription.payment_status !== 'PAID' || !subscription.paid_at) return { allowed:false, reason:'engine_subscription_payment_required', subscription, available_plans, payment_asset:'USDC', payment_network:'SOLANA' };
  if (!subscription.period_end || new Date(subscription.period_end) <= now) return { allowed:false, reason:'engine_subscription_expired', subscription, available_plans, payment_asset:'USDC', payment_network:'SOLANA' };
  return { allowed:true, reason:'engine_subscription_active', subscription, available_plans, payment_asset:'USDC', payment_network:'SOLANA' };
}
...
export async function createMemberEngineCheckout(pool, userId, planCode) {
  ...
  const planQ = await pool.query(`SELECT plan_id,code,name,billing_period_days,price_usdc,base_price_usdc,discount_bps,currency,payment_network FROM member_engine_plans WHERE code=$1 AND active=true LIMIT 1`,[planCode]);
  const plan = planQ.rows[0];
  if (!plan) throw new Error('engine_plan_not_found');
  if (plan.currency !== 'USDC' || plan.payment_network !== 'SOLANA') throw new Error('engine_plan_payment_rail_invalid');
  ...
  `,[userId,plan.plan_id]);
  if (existingQ.rows[0]) return { subscription:existingQ.rows[0], plan, reused:true, payment_asset:'USDC', payment_network:'SOLANA' };

  const created = await pool.query(`
    INSERT INTO member_engine_subscriptions(user_id,plan_id,status,payment_status,amount_usdc,payment_provider)
    VALUES($1,$2,'PENDING','PENDING',$3,'SOLANA_USDC')
  ...
  return { subscription:created.rows[0], plan, reused:false, payment_asset:'USDC', payment_network:'SOLANA' };
}
...
export async function activatePaidMemberEngineSubscription(pool, { subscriptionId, paymentProvider, paymentReference, paidAt = new Date() }) {
  ...
  if (paymentProvider !== 'SOLANA_USDC') throw new Error('unsupported_engine_payment_provider');
  if (!paymentReference) throw new Error('payment_reference_required');
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const q = await client.query(`
      SELECT s.subscription_id,s.user_id,s.plan_id,s.status,s.payment_status,p.billing_period_days,p.currency,p.payment_network
    ...
    if (row.currency !== 'USDC' || row.payment_network !== 'SOLANA') throw new Error('engine_plan_payment_rail_invalid');
