...
function envelope(provenance) {
  return {
    schema: 'aether.automatic_evidence.cross_source_corroboration.v2',
...
export function corroborateAutomaticEvidenceV2({ rpcEvidence, solscanEvidence, observedAt }) {
  ...
  return envelope({
    schema: 'aether.automatic_evidence.cross_source_corroboration.provenance.v2',
    algorithm: 'EXACT_SIGNATURE_SLOT_MATCH_WITH_UPSTREAM_VERIFICATION_V1',
...
export function verifyAutomaticEvidenceCorroborationV2(evidence) {
  if (!plain(evidence) || evidence.schema !== 'aether.automatic_evidence.cross_source_corroboration.v2') return false;
  try {
    ...
    const p = evidence.provenance;
    if (!plain(p) || p.schema !== 'aether.automatic_evidence.cross_source_corroboration.provenance.v2' || p.algorithm !== 'EXACT_SIGNATURE_SLOT_MATCH_WITH_UPSTREAM_VERIFICATION_V1') return false;
