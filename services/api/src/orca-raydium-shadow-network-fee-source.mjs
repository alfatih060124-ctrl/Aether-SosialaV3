...
function conservativeSolUsd(scan) {
  ...
  const prices = [];
  for (const pool of Array.isArray(scan.pools) ? scan.pools : []) {
    ...
    for (const value of [pool.buy_price_usd, pool.sell_price_usd]) {
      const price = Number(value); if (Number.isFinite(price) && price > 0) prices.push(price);
    }
  }
...
export function createOrcaRaydiumShadowNetworkFeeSource({ rpcUrl, scannerRuntime, fetchImpl = globalThis.fetch, usdcMint = DEFAULT_USDC_MINT, feePayer = DEFAULT_FEE_PAYER, timeoutMs = 5_000 } = {}) {
  const endpoint = text(rpcUrl, 'shadow_network_fee_rpc_url_required');
  if (!endpoint.startsWith('https://')) throw new Error('shadow_network_fee_rpc_https_required');
  ...
  return async function loadNetworkFeeEvidence() {
    ...
    const feeResult = await rpcCall(fetchImpl, endpoint, 'getFeeForMessage', [Buffer.from(message.serialize()).toString('base64'), { commitment: 'confirmed' }], timeoutMs);
    ...
    if (!(networkFeeUsdc > 0)) throw new Error('shadow_network_fee_usdc_invalid');
    return Object.freeze({ network_fee_usdc: networkFeeUsdc, network_fee_verified: true,
      source: 'SOLANA_GET_FEE_FOR_MESSAGE_CONSERVATIVE_SHADOW_BOUND',
      source_reference: `BLOCKHASH_${blockhash}|CU_${MAX_COMPUTE_UNITS}|P90_${priorityMicroLamports}|SOL_USD_ORCA_RAYDIUM`,
      read_only: true, transaction_signed: false, signer_requested: false, network_submission_authorized: false, live_execution_authorized: false });
  };
}

export const ORCA_RAYDIUM_SHADOW_NETWORK_FEE_SOURCE = Object.freeze({
  mode: 'SHADOW', strategy: 'TWO_LEG_ARBITRAGE', message_fee_source: 'getFeeForMessage',
  signing_authorized: false, network_submission_authorized: false, live_execution_authorized: false
});
