...
function envelope(provenance) {
  return {
    schema: 'aether.automatic_evidence.cross_source_corroboration.v3',
...
export function corroborateAutomaticEvidenceV3({ rpcEvidence, solscanEvidence, observedAt }) {
  ...
  return envelope({
    schema: 'aether.automatic_evidence.cross_source_corroboration.provenance.v3',
...
export function verifyAutomaticEvidenceCorroborationV3(evidence) {
  if (!plain(evidence) || evidence.schema !== 'aether.automatic_evidence.cross_source_corroboration.v3') return false;
  try {
    ...
    const p = evidence.provenance;
    if (!plain(p) || p.schema !== 'aether.automatic_evidence.cross_source_corroboration.provenance.v3' || p.algorithm !== 'EXACT_SIGNATURE_SLOT_MATCH_WITH_INDEPENDENT_UPSTREAM_VERIFICATION_V1') return false;
